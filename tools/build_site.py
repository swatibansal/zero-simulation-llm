...
STYLE = """
:root { --ink: #1f2430; --muted: #5b6472; --line: #e3e6ea; --accent: #1f77b4; --out-bg: #0e1420; }
* { box-sizing: border-box; }
body { margin: 0; color: var(--ink); background: #fff;
  font: 16px/1.65 -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }
.wrap { display: grid; grid-template-columns: 250px minmax(0, 900px); gap: 2.5rem;
  max-width: 1220px; margin: 0 auto; padding: 0 1.25rem; }
nav { position: sticky; top: 0; align-self: start; padding: 1.5rem 0; font-size: .85rem;
  max-height: 100vh; overflow-y: auto; }
nav a { display: block; color: var(--muted); text-decoration: none; padding: .28rem .6rem;
  border-left: 2px solid var(--line); }
nav a:hover { color: var(--ink); }
nav a.active { color: var(--accent); border-left-color: var(--accent); font-weight: 600; }
header.hero { grid-column: 1 / -1; padding: 3rem 0 2rem; border-bottom: 1px solid var(--line); }
header.hero h1 { font-size: 1.9rem; margin: 0 0 .4rem; }
header.hero p.tag { color: var(--muted); max-width: 62rem; margin: .3rem 0 1rem; }
header.hero .links a { color: var(--accent); text-decoration: none; margin-right: 1.2rem; font-weight: 600; }
.claims { display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: .8rem; margin-top: 1.2rem; }
.claims div { border: 1px solid var(--line); border-radius: 8px; padding: .8rem 1rem; font-size: .9rem; }
.claims strong { display: block; color: var(--accent); margin-bottom: .25rem; }
main { padding: 1rem 0 4rem; min-width: 0; }
h2 { margin-top: 3rem; padding-top: 1rem; border-top: 1px solid var(--line); scroll-margin-top: 1rem; }
blockquote { margin: 1rem 0; padding: .6rem 1.1rem; border-left: 3px solid var(--accent);
  background: #f4f8fb; color: var(--ink); }
table { border-collapse: collapse; margin: 1rem 0; font-size: .85rem; display: block; overflow-x: auto; }
th, td { border: 1px solid var(--line); padding: .4rem .65rem; text-align: left; }
th { background: #f6f8fa; }
p > code, li > code, td > code, blockquote code { background: #f0f2f5; border-radius: 4px;
  padding: .1rem .3rem; font-size: .85em; }
.cell { margin: 1.4rem 0; }
.cell .label { display: block; font: 600 .7rem/1 ui-monospace, monospace; color: var(--muted);
  text-transform: uppercase; letter-spacing: .06em; margin-bottom: .3rem; }
.cell .in pre { margin: 0; padding: .9rem 1rem; border: 1px solid var(--line); border-radius: 8px;
  overflow-x: auto; font-size: .8rem; line-height: 1.5; background: #f6f8fa; }
.cell .out { margin-top: .6rem; }
.cell .out pre { margin: 0; padding: .9rem 1rem; border-radius: 8px; overflow-x: auto;
  font-size: .78rem; line-height: 1.5; background: var(--out-bg); color: #d8e1ec; }
...
HERO = f"""
<header class="hero">
  <h1>zero-sim: ZeRO-1 / ZeRO-2 / ZeRO-3 on 32 virtual GPUs</h1>
  ...
  <div class="claims">
    <div><strong>Same math, bit for bit</strong>All four strategies produce bitwise-identical
    weights after every step — sharding changes where bytes live, never the result.</div>
    <div><strong>Memory: 16Ψ → 16Ψ/N</strong>Measured per-GPU memory matches the ZeRO paper's
    formulas exactly, from full replication down to fully sharded.</div>
    <div><strong>Traffic: 2Ψ vs 3Ψ</strong>ZeRO-1/2 move the same bytes as plain data parallel;
    ZeRO-3 pays 1.5× for gathering parameters just in time.</div>
  </div>
</header>
"""
