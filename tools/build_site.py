...
header.hero { grid-column: 1 / -1; padding: 4rem 0 2.5rem; border-bottom: 1px solid var(--line);
  background:
    radial-gradient(ellipse 60% 90% at 15% -10%, rgba(94, 178, 255, .14), transparent),
    radial-gradient(ellipse 50% 80% at 85% -20%, rgba(74, 222, 128, .08), transparent); }
header.hero h1 { font-size: 2.1rem; line-height: 1.25; margin: 0 0 .6rem; color: var(--bright);
  letter-spacing: -.01em; }
header.hero h1 span { color: var(--accent); }
header.hero p.tag { color: var(--muted); max-width: 56rem; margin: .3rem 0 1.3rem; font-size: 1.02rem; }
header.hero .links a { text-decoration: none; margin-right: 1.5rem; font-weight: 600; font-size: .92rem; }
